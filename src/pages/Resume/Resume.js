// import React from "react";
// import "/w3css/4/w3.css";
// import "/cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css";
// import "./resume.css";

// export default function Resume() {
//   // return (
//   //   <div class="full">
//   //     <div class="left">
//   //       <div class="Contact">
//   //         <h2 className="resume-title">Contact</h2>
//   //         <ul style={{ fontFamily: "monospace", fontSize: "15px" }}>
//   //           <li>
//   //             <b>E-mail ID: </b>
//   //           </li>
//   //           <li style={{ marginLeft: "2rem" }}>sujeeth.ct20@bitsathy.ac.in</li>
//   //           <li style={{ marginLeft: "2rem" }}>sujeethkrr.com</li>
//   //           <li></li>
//   //           <li>
//   //             <b>Phone Number: </b> 8072155023
//   //           </li>
//   //         </ul>
//   //       </div>
//   //       <div class="Skills">
//   //         <h2 className="resume-title">Skills</h2>
//   //         <ul>
//   //           <li style={{ fontWeight: "bolder", fontSize: "20px" }}>
//   //             Programming Skills:
//   //           </li>
//   //           <li style={{ fontFamily: "monospace", fontSize: "15px" }}>
//   //             <li>C,</li>
//   //             <li>C++,</li>
//   //             <li>Java</li>
//   //           </li>
//   //           <li style={{ fontWeight: "bolder", fontSize: "20px" }}>
//   //             Front-end Skills:
//   //           </li>
//   //           <li style={{ fontFamily: "monospace", fontSize: "15px" }}>
//   //             <li>HTML,</li>
//   //             <li>CSS,</li>
//   //             <li>Javascript,</li>
//   //             <li>React</li>
//   //           </li>
//   //           <li style={{ fontWeight: "bolder", fontSize: "20px" }}>
//   //             Back-end Skills:
//   //           </li>
//   //           <li style={{ fontFamily: "monospace", fontSize: "15px" }}>
//   //             <li>MySQL,</li>
//   //             <li>MongoDB,</li>
//   //             <li>Firebase,</li>
//   //             <li>Node</li>
//   //           </li>
//   //         </ul>
//   //       </div>
//   //       <div class="Language">
//   //         <h2 className="resume-title">Language</h2>
//   //         <ul>
//   //           <li style={{ fontFamily: "monospace", fontSize: "15px" }}>
//   //             English
//   //           </li>
//   //           <li style={{ fontFamily: "monospace", fontSize: "15px" }}>Tamil</li>
//   //         </ul>
//   //       </div>
//   //       <div class="Hobbies">
//   //         <h2 className="resume-title">Hobbies</h2>
//   //         <ul>
//   //           <li style={{ fontFamily: "monospace", fontSize: "15px" }}>
//   //             Playing and watching cricket
//   //           </li>
//   //           <li style={{ fontFamily: "monospace", fontSize: "15px" }}>
//   //             Newspaper reading
//   //           </li>
//   //         </ul>
//   //       </div>
//   //     </div>
//   //     <div class="right">
//   //       <div class="name">
//   //         <h1 className="resume-title">SUJEETH G</h1>
//   //       </div>
//   //       <div class="title">
//   //         <ul>
//   //           <li>
//   //             <b>Front-end Developer</b>
//   //           </li>
//   //         </ul>
//   //       </div>
//   //       <div class="Summary">
//   //         <h2 className="resume-title">Career Objective</h2>
//   //         <ul>
//   //           <li>Persistent, eagerly waiting to join as Software Developer to provide better solutions for implementing and developing applications along with my Technical and Designing skills.</li>
//   //         </ul>
//   //       </div>
//   //       <div class="Experience">
//   //         <h2 className="resume-title">Experience</h2>
//   //         <h3>Abc webdev pvt ltd - Senior Web Developer</h3>
//   //         <p>January 2022 to Present</p>
//   //         <ul>
//   //           <li>Actively engaged in web creative design and development.</li>
//   //           <li>Designing project & planning</li>
//   //         </ul>
//   //         <h3>Xyz webdev pvt ltd - junior web developer</h3>
//   //         <p>August 2021 to December 2021</p>
//   //         <ul>
//   //           <li>Actively engaged in web creative design and development.</li>
//   //           <li>Designing project & planning</li>
//   //           <li>Working on designing</li>
//   //         </ul>
//   //       </div>
//   //       <div class="Education">
//   //         <h2 className="resume-title">Education</h2>
//   //         <table>
//   //           <tr>
//   //             <th>University/college </th>
//   //             <th>Passing year </th>
//   //             <th>percentage/cgpa</th>
//   //           </tr>
//   //           <tr>
//   //             <td>xyz</td>
//   //             <td>2020</td>
//   //             <td>8.9</td>
//   //           </tr>
//   //           <tr>
//   //             <td>pqr</td>
//   //             <td>2018</td>
//   //             <td>89%</td>
//   //           </tr>
//   //         </table>
//   //       </div>
//   //     </div>
//   //   </div>
//   // );
//   return (
//     <div>
//       <div class="w3-content w3-margin-top" style="max-width:1400px;">
//         <div class="w3-row-padding">
//           <div class="w3-third">
//             <div class="w3-white w3-text-grey w3-card-4">
//               <div class="w3-container">
//                 <p>
//                   <i class="fa fa-briefcase fa-fw w3-margin-right w3-large w3-text-teal"></i>
//                   Front-end developer
//                 </p>
//                 <p>
//                   <i class="fa fa-home fa-fw w3-margin-right w3-large w3-text-teal"></i>
//                   Karur, TamilNadu
//                 </p>
//                 <p>
//                   <i class="fa fa-envelope fa-fw w3-margin-right w3-large w3-text-teal"></i>
//                   sujeethkrr@gmail.com
//                 </p>
//                 <p>
//                   <i class="fa fa-phone fa-fw w3-margin-right w3-large w3-text-teal"></i>
//                   91+ 8072155023
//                 </p>
//                 <hr />

//                 <p class="w3-large">
//                   <b>
//                     <i class="fa fa-asterisk fa-fw w3-margin-right w3-text-teal"></i>
//                     Programming Skills
//                   </b>
//                 </p>
//                 <p>C</p>
//                 <p>C++</p>
//                 <p>Java</p>

//                 <p class="w3-large">
//                   <b>
//                     <i class="fa fa-asterisk fa-fw w3-margin-right w3-text-teal"></i>
//                     Front-end Skills
//                   </b>
//                 </p>
//                 <p>HTML</p>
//                 <p>CSS</p>
//                 <p>Javascript</p>
//                 <p>React</p>

//                 <p class="w3-large">
//                   <b>
//                     <i class="fa fa-asterisk fa-fw w3-margin-right w3-text-teal"></i>
//                     Back-end Skills
//                   </b>
//                 </p>
//                 <p>MySQL</p>
//                 <p>MongoDB</p>
//                 <p>Node</p>
//                 <p>Firebase</p>
//                 <hr />

//                 <p class="w3-large w3-text-theme">
//                   <b>
//                     <i class="fa fa-globe fa-fw w3-margin-right w3-text-teal"></i>
//                     Languages
//                   </b>
//                 </p>
//                 <p>English</p>
//                 <p>Tamil</p>
//                 <br />
//               </div>
//             </div>
//             <br />
//           </div>
//           <div class="w3-twothird">
//             <div class="w3-container w3-card w3-white w3-margin-bottom">
//               <h2 class="w3-text-grey w3-padding-16">
//                 <i class="fa fa-suitcase fa-fw w3-margin-right w3-xxlarge w3-text-teal"></i>
//                 Work Experience
//               </h2>
//               <div class="w3-container">
//                 <h5 class="w3-opacity">
//                   <b>UI & UX Designer / TrusTrace</b>
//                 </h5>
//                 <h6 class="w3-text-teal">
//                   <i class="fa fa-calendar fa-fw w3-margin-right"></i>June 2022
//                 </h6>
//                 <p>
//                   Worked as an designer in developing fake certificate
//                   detection.
//                 </p>
//                 <p>Built overall interface using Figma.</p>
//                 <hr />
//               </div>
//             </div>

//             <div class="w3-container w3-card w3-white w3-margin-bottom">
//               <h2 class="w3-text-grey w3-padding-16">
//                 <i class="fa fa-suitcase fa-fw w3-margin-right w3-xxlarge w3-text-teal"></i>
//                 Recent Achievements
//               </h2>
//               <div class="w3-container">
//                 <h5 class="w3-opacity">
//                   <b>BIT'S HACK 2022</b>
//                 </h5>
//                 <h6 class="w3-text-teal">
//                   <i class="fa fa-calendar fa-fw w3-margin-right"></i>Feb - 2023
//                 </h6>
//                 <p>
//                   Awarded <b>Special mention award</b> for developing project
//                   under the title <b>Result Intimation System</b> conducted by
//                   Bannari Amman Institute of Technology
//                 </p>
//                 <hr />
//               </div>
//             </div>

//             <div class="w3-container w3-card w3-white">
//               <h2 class="w3-text-grey w3-padding-16">
//                 <i class="fa fa-certificate fa-fw w3-margin-right w3-xxlarge w3-text-teal"></i>
//                 Education
//               </h2>
//               <div class="w3-container">
//                 <h5 class="w3-opacity">
//                   <b>B-Tech</b>
//                 </h5>
//                 <h6 class="w3-text-teal">
//                   <i class="fa fa-calendar fa-fw w3-margin-right"></i>2020 -
//                   2024
//                 </h6>
//                 <p>Bannari Amman Institute of Technology</p>
//                 <hr />
//               </div>
//               <div class="w3-container">
//                 <h5 class="w3-opacity">
//                   <b>Little Angals' English Hr Sec Sechhol</b>
//                 </h5>
//                 <h6 class="w3-text-teal">
//                   <i class="fa fa-calendar fa-fw w3-margin-right"></i>2018 -
//                   2020
//                 </h6>
//                 <p>Higher Secondary</p>
//                 <hr />
//               </div>
//               <div class="w3-container">
//                 <h5 class="w3-opacity">
//                   <b>Little Angals' English Hr Sec Sechhol</b>
//                 </h5>
//                 <h6 class="w3-text-teal">
//                   <i class="fa fa-calendar fa-fw w3-margin-right"></i>209 - 2018
//                 </h6>
//                 <p>Secondary</p>
//                 <br />
//               </div>
//             </div>
//           </div>
//         </div>
//       </div>

//       <footer class="w3-container w3-teal w3-center w3-margin-top">
//         <p>Find me on social media.</p>
//         <i class="fa fa-facebook-official w3-hover-opacity"></i>
//         <i class="fa fa-instagram w3-hover-opacity"></i>
//         <i class="fa fa-snapchat w3-hover-opacity"></i>
//         <i class="fa fa-pinterest-p w3-hover-opacity"></i>
//         <i class="fa fa-twitter w3-hover-opacity"></i>
//         <i class="fa fa-linkedin w3-hover-opacity"></i>
//         <p>
//           Powered by{" "}
//           <a href="https://www.w3schools.com/w3css/default.asp" target="_blank">
//             w3.css
//           </a>
//         </p>
//       </footer>
//     </div>
//   );
// }


import React from 'react';
import './resume1.css';

const Resume = () => {
  return (
    <div className="Resume">
      <div className="header">
        <h1>SUJEETH G</h1>
        <h2>Web Developer</h2>
      </div>
      <div className="contact">
        <ul>
          <li><i className="fas fa-phone"></i> (91+) 8072155023</li>
          <li><i className="fas fa-envelope"></i> sujeethkrr@gmail.com</li>
          <li><i className="fas fa-envelope"></i> sujeeth.ct20@bitsathy.ac.in</li>
          <li><i className="fas fa-globe"></i> johndoe.com</li>
        </ul>
      </div>
      <div className="summary">
        <h3>About me</h3>
        <p>
        Persistent, eagerly waiting to join as Software Developer to provide better solutions for implementing and developing applications along with my Technical and Designing skills.
        </p>
      </div>
      
      <div className="experience">
        <h3>Experience</h3>
        <div className="job">
          <h4>UI / UX Designer</h4>
          <h5>TrusTrace</h5>
          <h6>June 2022</h6>
          <ul>
            <li>Worked on designing the frontend part for fake certificate detection.</li>
          </ul>
        </div>
      </div>
      <div className="education">
        <h3>Education</h3>
        <div className="degree">
          <h4>B-Tech, Computer Technology</h4>
          <h5>Bannari Amman Institute of Technology</h5>
          <h6>2020 - 2024</h6>
        </div>
      </div>
      <div className="achieve">
        <h3>Achievements</h3>
        <div className="degree">
          <h4>Special mention Award</h4>
          <ul>
            <li>Participated in project presentation conducted by Bannari Amman Institute of Technology</li>
          </ul>
          <h6>2023</h6>
        </div>
      </div>
    </div>
  );
}

export default Resume;


